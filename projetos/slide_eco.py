...
sudo apt install python3-pip espeak espeak-data espeak-data-pt espeak-data-pt-br festival festvox-kallpc16k tesseract-ocr tesseract-ocr-por xdotool wmctrl xclip
"""
...
"""
    ...
    def generate_audio(self, text):
        ...
        try:
            ...
            # Gerar áudio com gTTS (usando 'pt' para português)
            ...
            try:
                ...
                for variant in espeak_variants:
                    try:
                        subprocess.run(['espeak', '-v', variant, '-s', '150', '-w', str(audio_file), clean_text], 
                ...
                subprocess.run(['espeak', '-s', '150', '-w', str(audio_file), clean_text], 
                             check=True, timeout=10)
                return audio_file
                
            except Exception as e2:
                logger.error(f"Erro com espeak também: {e2}")
                # Último fallback: usar festival
                try:
                    ...
                    subprocess.run(['text2wave', str(text_file), '-o', str(audio_file)], 
                                 check=True, timeout=15)
                    text_file.unlink()  # Limpar arquivo de texto
                    return audio_file
                except Exception as e3:
                    logger.error(f"Erro com festival também: {e3}")
                    return None
